...
function init_GUI() {
  ...
  function USE_HDRIChanger() {
    changeHDRI_Type = true;
  }
  function HDRIColorChanger() {
    changeHDRIColor = true;
  }
  ...
  HDRI_ExposureController = gui
    .add(HDRI_ExposureObject, "HDRI_Exposure", 0, 3, 0.05)
    .onChange(HDRI_ExposureChanger);
  HDRI_TypeController = gui
    .add(HDRI_TypeObject, "use_HDRI", 0, 1, 1)
    .onChange(USE_HDRIChanger);

  material_TypeController = gui
    .add(material_TypeObject, "Material_Type", 2, 4, 1)
    .onChange(materialTypeChanger);
  material_ColorController = gui
    .addColor(material_ColorObject, "Material_Color")
    .onChange(materialColorChanger);

  HDRI_ColorController = gui
    .addColor(HDRI_ColorObject, "HDRI_Color")
    .onChange(HDRIColorChanger);
  material_RoughnessController = gui
    .add(material_RoughnessObject, "Material_Roughness", 0.0, 1.0, 0.01)
    .onChange(materialRoughnessChanger);
...
function load_GLTF_Model() {
  ...
  gltfLoader.load(
    "models/david.gltf",
    function (
      meshGroup // Triangles: 100,000 // if you choose to load in the different models below, scroll down and change the *GLTF model settings* for this particular model //gltfLoader.load("models/TronTank.gltf", function( meshGroup ) // Triangles: 17,533 //gltfLoader.load("models/StanfordBunny.glb", function( meshGroup ) // Triangles: 30,338
    ) {
      ...
      meshGroup.traverse(function (child) {
        if (child.isMesh) {
          ...
          triangleMaterialMarkers.push(
            child.geometry.attributes.position.array.length / 9
          );
          meshList.push(child);
        }
      });
      ...
      console.log(modelMesh.geometry.attributes.normal);
      modelMesh.geometry.computeVertexNormals(true);
      console.log(modelMesh.geometry.attributes.normal);
      modelMesh.geometry =
        THREE.BufferGeometryUtils.mergeBufferGeometries(geoList);
      ...
      for (let i = 0; i < uniqueMaterialTextures.length; i++) {
        for (let j = i + 1; j < uniqueMaterialTextures.length; j++) {
          if (
            uniqueMaterialTextures[i].image.src ==
...
function initSceneData() {
  ...
  cameraControlsObject.position.set(18, 41, -8);
  ...
  for (let i = 0; i < total_number_of_triangles; i++) {
    ...
    triangle_b_box_centroid.set(
      (triangle_b_box_min.x + triangle_b_box_max.x) * 0.5,
  ...
  //   hdrPath = "textures/photo_studio_loft_hall_4k.hdr";
  ...
  hdrTexture = hdrLoader.load(hdrPath, function (texture, textureData) {
    ...
    console.log(
      "brightest texel index: " + texel + " | max luminance value: " + max
    );
    ...
    console.log(sunDirectionVector);
  });

  init_GUI();

  // scene/demo-specific uniforms go here
  pathTracingUniforms.tTriangleTexture = {
    type: "t",
    value: triangleDataTexture,
  };
  ...
  pathTracingUniforms.uUSE_HDRI = { type: "i", value: 0 };
  ...
  pathTracingUniforms.uHDRIColor = { type: "v3", value: new THREE.Color() };
  pathTracingUniforms.uSunDirectionVector = {
    type: "v3",
    value: sunDirectionVector,
  };
...
function updateVariablesAndUniforms() {
  if (changeHDRI_Exposure) {
    ...
    pathTracingUniforms.uHDRI_Exposure.value =
      HDRI_ExposureController.getValue();
    cameraIsMoving = true;
    changeHDRI_Exposure = false;
  }

  if (changeMaterialType) {
    pathTracingUniforms.uMaterialType.value =
      material_TypeController.getValue();
    cameraIsMoving = true;
    changeMaterialType = false;
  }

  if (changeHDRI_Type) {
    pathTracingUniforms.uUSE_HDRI.value = HDRI_TypeController.getValue();
    cameraIsMoving = true;
    changeHDRI_Type = false;
  }

  if (changeMaterialColor) {
    matColor = material_ColorController.getValue();
    pathTracingUniforms.uMaterialColor.value.setRGB(
      matColor[0],
      matColor[1],
      matColor[2]
    );
  ...
  if (changeHDRIColor) {
    hdriColor = HDRI_ColorController.getValue();
    pathTracingUniforms.uHDRIColor.value.setRGB(
      hdriColor[0],
      hdriColor[1],
      hdriColor[2]
    );
    ...
    cameraIsMoving = true;
    changeHDRIColor = false;
  }

  if (changeMaterialRoughness) {
    pathTracingUniforms.uRoughness.value =
      material_RoughnessController.getValue();
    cameraIsMoving = true;
